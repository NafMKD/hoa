...
const demoStickers: Sticker[] = Array.from({ length: 20 }, (_, i) => ({
  id: i + 1,
  vehicle: `Vehicle ${i + 1}`,
  stickerNumber: `STK-${1000 + i}`,
  expiryDate: `2025-12-${(i % 28) + 1}`,
  status: i % 2 === 0 ? "Valid" : "Expired",
}));

const columns: ColumnDef<Sticker>[] = [
  { accessorKey: "vehicle", header: "Vehicle" },
  { accessorKey: "stickerNumber", header: "Sticker Number" },
  { accessorKey: "expiryDate", header: "Expiry Date" },
  { accessorKey: "status", header: "Status", cell: ({ row }) => (
    <span className={`px-2 py-1 rounded text-xs font-medium ${row.original.status === "Valid" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>{row.original.status}</span>
  )}
];
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex space-x-2">
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button variant="outline"><Plus className="mr-2 h-4 w-4" /> Add Sticker</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Add Sticker</DialogTitle>
                <DialogDescription>Fill in sticker information below.</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-2">
                <Input placeholder="Vehicle" />
                <Input placeholder="Sticker Number" />
        ...
        <Input
          placeholder="Search vehicle or sticker..."
          value={(table.getColumn("vehicle")?.getFilterValue() as string) ?? ""}
          onChange={(e) => table.getColumn("vehicle")?.setFilterValue(e.target.value)}
          className="max-w-sm"
        />
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              ...
              <TableRow><TableCell colSpan={columns.length} className="h-24 text-center">No stickers found.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
